...
import {PHRASE, showPlayerResult} from './showPlayerResult';

const resultAllPlayer = [10, 12, 15, 19];

describe(`The player has lost`, () => {
  it(`Over time`, () => {
    const playerResult = {remainingTime: 0, remainingNotes: 3, points: 5};
    assert.equal(showPlayerResult(resultAllPlayer, playerResult), PHRASE.failTime);
  });
  it(`Ended attempts`, () => {
    const playerResult = {remainingTime: 25, remainingNotes: -1, points: 15};
...
describe(`The player wins`, () => {
  it(`The player ranked first`, () => {
    const playerResult = {remainingTime: 25, remainingNotes: 1, points: 20};
    assert.equal(showPlayerResult(resultAllPlayer, playerResult), PHRASE.win(1, 5, 80));
  });
  it(`The player ranked last`, () => {
    const playerResult = {remainingTime: 25, remainingNotes: 1, points: 5};
  ...
  it(`The player does not first and not last place`, () => {
    const playerResult = {remainingTime: 25, remainingNotes: 1, points: 18};
